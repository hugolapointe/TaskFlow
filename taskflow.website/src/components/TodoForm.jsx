...
const TodoForm = () => {
  const { selectedTodo, createTodo, updateTodo, clearSelection, togglePriority } = useTodos();
...
const handleSubmit = async (e) => {
  ...
  await updateTodo(selectedTodo.id, {
        ...
        if (priorityChanged) {
          await togglePriority(selectedTodo.id, true);
        }
   ...
   className={`flex-1 px-4 py-3 rounded-xl border-2 transition-all duration-200 flex items-center justify-center gap-2 font-medium ${
    isPriority
       ...
       className={`w-5 h-5 transition-all ${
            isPriority ? 'fill-amber-400 scale-110' : 'fill-gray-400'
    }`} 
  viewBox="0 0 24 24"
  >
      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
