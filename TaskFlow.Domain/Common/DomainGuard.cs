...
public static class DomainGuard {
    ...
    public static class Against {
        ...
        public static void InvalidLength(string value, string paramName, int maxLength, int minLength = 0) {
            if (value.Length < minLength) throw new ArgumentException("Value is too short.", paramName);
            if (value.Length > maxLength) throw new ArgumentException("Value exceeds max length.", paramName);
        }
