...
*   **Task Management:** Use the TodoWrite tool to track and manage complex multi-step tasks, ensuring transparency and progress visibility.
*   **Planning Mode:** For significant or complex changes, use EnterPlanMode to explore the codebase, design an implementation approach, and get user approval before implementation.
*   **Tool Usage:** Leverage specialized tools (Read, Edit, Write, Glob, Grep) for file operations rather than bash commands when possible.
*   **Parallel Execution:** When performing multiple independent operations (e.g., reading multiple files, running multiple searches), execute them in parallel within a single response for optimal efficiency.
*   **Context Management:** Claude Code has unlimited context through automatic summarization, enabling comprehensive analysis and complete task execution without artificial limitations.
